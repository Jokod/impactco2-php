...
class HeaterEnpoint extends Endpoint
{
     ...
     *
     * @param int $surface Number of square meters on which to calculate emissions. If not specified, use the average size of an apartment in France: 63m2
     * @param int[] $types List of the type of heating
     */
    public function __construct(int $surface = null, array $types = null)
    {
        ...
        if (!is_null($types)) {
            ...
            foreach ($types as $type) {
                if (!in_array($type, HeaterEnum::toArray())) {
